  ...
  async getDashboardStats(): Promise<ApiResponse<DashboardStats>> {
    ...
    try {
      ...
      return {
        success: true,
        data: {
          total_available_cards: stats.available_cards || 0,
          total_delivered_cards: stats.delivered_cards || 0,
          total_assigned_cards: stats.assigned_cards || 0,
          total_cards: stats.total_cards || 0,
          recent_activity: (stats.recent_activity || []).map((activity: any) => ({
            id: activity.id || 0,
            action: activity.assigned_at ? "assigned" : "delivered",
            card_serial: activity.card_serial,
            customer_name: activity.customer_name,
            timestamp: activity.assigned_at || activity.delivered_at || new Date().toISOString(),
          })),
        },
      };
    } catch (error: any) {
